...
export default class AdminSummaries extends Component {
    ...
    requestList = () => {
        axios.requestList(this, '/public/summarys/page', this.params)
    }
    ...
    handleAddSummary = () => {
        this.props.history.push('/admin/createSummary')
    }
    ...
    handleUpdate = () => {
        ...
        if (selectRecord) {
            this.props.history.push(`/admin/updateSummary/${selectRecord.id}`)
        }
    }
    ...
    handleRowDelete = () => {
        ...
        if (selectRecord) {
            axios.ajax({
                url: Api.Urls.adminDelSummary + selectRecord.id,
                method: 'post'
            }).then((res) => {
                this.requestList()
                message.success("删除个人小结成功!")
            })
        }
    }
    ...
    render() {
            ...
            render: () => {
                return (
                    <span>
                        ...
                        <Popconfirm title="确定要删除这个小结吗？" onConfirm={this.handleRowDelete}>
        ...
        return <div>
            ...
            <Card>
                <Button type='primary' style={{marginRight: 10}} onClick={this.handleAddSummary}>发布总结</Button>
            </Card>
